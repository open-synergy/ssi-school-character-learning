...
> **Extends:** ssi_school_character_growth_report — model
> `school_character_growth_report`, aksi `01-create`
...
- **Operating Unit**: The operating unit this Character Growth Report document
  belongs to. Not required by the field definition itself, but a document without
  an Operating Unit is invisible to users who are only granted the Operating Unit
...
- The Character Growth Report list/form is filtered by an `ir.rule` for users in
  the *Operating Unit* group: they can only read/write/create/unlink documents
  whose **Operating Unit** is one of the operating units assigned to them
